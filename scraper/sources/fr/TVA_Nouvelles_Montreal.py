...
class TVA_Nouvelles_Montreal:
  ...
  def get_story_author(self):
    ...
    try:
      story_author = self.article_content.find(
          attrs={'name': 'cXenseParse:author'})['content']
    except Exception:
      message_to_log = traceback.format_exc()
      tracing.log(current_module, 'exception', message_to_log)
    return story_author
  ...
  def get_story_publication_date(self):
    ...
    try:
      story_publication_date = self.article_content.find(
          attrs={'itemprop': 'datePublished'})['datetime']
    except Exception:
      message_to_log = traceback.format_exc()
      tracing.log(current_module, 'exception', message_to_log)
  ...
  def get_story_body(self):
    ...
    try:
      story_body = []
      for body_element in self.article_content.find_all(class_="story-body"):
        tags = ['p', 'h2']
        for paragraph in body_element.find_all(tags):
