...
class Le_Parisien:
  ...
  def get_story_author(self):
    ...
    try:
      story_author = []
      for author_element in self.article_content.find_all(class_='author'):
        for paragraph_or_element in author_element.find_all('span',
                                                            recursive=False):
          story_author.append(paragraph_or_element.get_text())
  ...
  def get_story_publication_date(self):
    ...
    try:
      story_publication_date = self.article_content.find_all(
  ...
  def get_story_body(self):
    ...
    try:
      story_body = []
      for body in self.article_content.find_all(class_="article-full__body"):
