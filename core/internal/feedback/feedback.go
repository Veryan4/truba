...
func FeedbackReceived(userFeedback UserFeedback) error {
	...
	user.UpdateFromStory(userFeedback.UserId, currentStory.Source.SourceId, *currentStory.Source.Name, reward, user.FAVORITE_SOURCE_DB_COLLECTION_NAME, *currentStory.Language)
	story.UpdateSourceReputation(currentStory.Source.SourceId, reward)
	user.UpdateFromStory(userFeedback.UserId, currentStory.Author.AuthorId.String(), *currentStory.Author.Name, reward, user.FAVORITE_AUTHOR_DB_COLLECTION_NAME, *currentStory.Language)
