...
func CreateAccessToken(data jwt.MapClaims, expiresDelta time.Duration) (string, error) {
	var expire time.Time
	if expiresDelta > 0 {
		expire = time.Now().Add(expiresDelta)
	} else {
		expire = time.Now().AddDate(0, 0, ACCESS_TOKEN_EXPIRE_DAYS)
	}
	claims := data
	claims["exp"] = expire
	t := jwt.NewWithClaims(jwt.GetSigningMethod(ENCRYPTION_ALGORITHM), claims)
	return t.SignedString(os.Getenv("JWT_SECRET"))
}
