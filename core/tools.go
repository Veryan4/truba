...
func main() {
	models := []string{"Source", "Author", "Keyword", "Entity", "Story", "DataSet", "Feature", "ScrapedUrl", "Search"}
...
func GenerateModel(modelName string) {
	...
	err = os.WriteFile("./pkg/models/"+strings.ToLower(modelName)+"_model.go", []byte(code), 0o644)
	if err != nil {
